...
public:
  ...
  int update(int target,const bool torque_only,int rpm_scaler=3)
  {
    ...
    const long ticks_per_rev=36L; // encoder ticks per motor revolution
    int target_time=(1000L*60)/(rpm*ticks_per_rev); // ms/tick = ms/sec * sec/min / rev/min * ticks/rev
