...
namespace nano_net
{
  ...
  void got_setup() { // read last_setup and do it
    /*for (int m=0;m<n_motors;m++)
    {
      ...
      if (mode!='T') {
        int src=mode-'0';*/
        speed_controllers[0]=speed_controller(&encoders[0]);
    /*  }
    }*/
  }
  ...
  */
  ...
  void send_motors(int8_t command) 
  {
      int8_t speed=0;
      speed=speed_controllers[0].update(command,false);
      send_motor_power(motors[0],speed);

      if(encoders[0].count_mono < prev) // 65535 is wrap point
      {
        temp_count = (65535 - prev) + encoders[0].count_mono;
        tick_count += uint64_t(temp_count);
      }
      else if(encoders[0].count_mono > prev)
      {
        tick_count += uint64_t(encoders[0].count_mono) - prev;
      }

      prev = encoders[0].count_mono;

      Serial.print("tick_count: ");
      Serial.println(int(tick_count));
...
{
  Serial.begin(115200);
  Serial.println("Hey, the nano's up");
