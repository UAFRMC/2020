...
int MPU_readSensor(int addrL, int addrH){
  byte L = MPU_readSensor(addrL);
  byte H = MPU_readSensor(addrH);

  return (int16_t)((H<<8)+L);
}

// Read a one-byte value from this address.
int MPU_readSensor(int addr){
  Wire.beginTransmission(MPU_I2C_TARGET);
  Wire.write(addr);
  Wire.endTransmission(false);

  Wire.requestFrom(MPU_I2C_TARGET, 1, true);
  return Wire.read();
}
...
void MPU_readings::setup(){
  ...
  MPU_writeSensor(0x0A, 0x0F); //SelfTest
  MPU_writeSensor(0x0A, 0x00); //PowerDownMode

  MPU_I2C_TARGET = MPU_I2C_ADDRESS;      //change Adress to MPU

  MPU_writeSensor(0x24, 0x40); //Wait for Data at Slave0
  MPU_writeSensor(0x25, 0x8C); //Set i2c address at slave0 at 0x0C
  MPU_writeSensor(0x26, 0x02); //Set where reading at slave 0 starts
  MPU_writeSensor(0x27, 0x88); //set offset at start reading and enable
  MPU_writeSensor(0x28, 0x0C); //set i2c address at slv1 at 0x0C
  MPU_writeSensor(0x29, 0x0A); //Set where reading at slave 1 starts
  MPU_writeSensor(0x2A, 0x81); //Enable at set length to 1
  MPU_writeSensor(0x64, 0x01); //override register
  MPU_writeSensor(0x67, 0x03); //set delay rate
  MPU_writeSensor(0x01, 0x80);

  MPU_writeSensor(0x34, 0x04); //set i2c slv4 delay
  MPU_writeSensor(0x64, 0x00); //override register
  MPU_writeSensor(0x6A, 0x00); //clear usr setting
  MPU_writeSensor(0x64, 0x01); //override register
...
MPU_readings MPU_readings::read(void) {
  MPU_readings imu;
  imu.A[0]=MPU_readSensor(MPU_ACCEL_XOUT_L,MPU_ACCEL_XOUT_H); // range default +-2g = +-32768
  imu.A[1]=MPU_readSensor(MPU_ACCEL_YOUT_L,MPU_ACCEL_YOUT_H);
  imu.A[2]=MPU_readSensor(MPU_ACCEL_ZOUT_L,MPU_ACCEL_ZOUT_H);

  imu.G[0]=MPU_readSensor(MPU_GYRO_XOUT_L,MPU_GYRO_XOUT_H);  // range +-250 deg/sec = +-32768
  imu.G[1]=MPU_readSensor(MPU_GYRO_YOUT_L,MPU_GYRO_YOUT_H);
  imu.G[2]=MPU_readSensor(MPU_GYRO_ZOUT_L,MPU_GYRO_ZOUT_H);

  imu.C[0]=MPU_readSensor(MPU_CMPS_XOUT_L,MPU_CMPS_XOUT_H);
  imu.C[1]=MPU_readSensor(MPU_CMPS_YOUT_L,MPU_CMPS_YOUT_H);
  imu.C[2]=MPU_readSensor(MPU_CMPS_ZOUT_L,MPU_CMPS_ZOUT_H);

  imu.T=( MPU_readSensor(MPU_TEMP_OUT_L,MPU_TEMP_OUT_H) )/34 + 365; // deg C * 10
  return imu;
}
...
void loop()
{
  ...
  Serial.print("   A ");
  for (int i=0;i<3;i++) { 
    Serial.print(imu.A[i]); 
    Serial.print(" "); 
  }

  Serial.print("  G  ");
  for (int i=0;i<3;i++) { 
    Serial.print(imu.G[i]); 
    Serial.print(" "); 
  }

  Serial.print("  C  ");
  for (int i=0;i<3;i++) { 
    Serial.print(imu.C[i]); 
    Serial.print(" "); 
  }
