...
{      
  ...
  pinMode(A5,OUTPUT); 
  digitalWrite(A5,HIGH); // Vref for IR sensors
  pinMode(A11,OUTPUT); 
...
void loop()
{
  // Infrared input lines:
  for (int ana=A0;ana<A11;ana++) {
    if (ana!=A5) { // valid reading:
      unsigned int v=analogRead(ana);
      Serial.print((int)(v)); // *100/1024));
    }
    Serial.print(" ");
  }
  ...
  // Battery
  Serial.print(" bat: ");
  unsigned int v=analogRead(A12);
  unsigned int mcu_mV=4940; // millivolts at MCU "5v" rail / AREF
  unsigned int mV=(v*long(11)*mcu_mV)/1024; 
  Serial.print(mV);
