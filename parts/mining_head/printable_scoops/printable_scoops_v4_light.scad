...
scoop_switchover_x=idler_dia/2+1.25*scoop_fillet;
...
module scoop_full() {
	extrameat=3.5; // total width of front cutting face
		...
		union() {
			...
			intersection() {
				union() {
					...
					translate([cutting_dia/2,0,0]) rotate([0,relief_angle*0.7,0]) translate([-cutting_dia/2,0,0]) 
				...
				scale([1,1,-1]) // extrude downward
					linear_extrude(height=30)
						...
						union() {
							...
							translate([cutting_dia/2-extrameat+10,0]) square([20,2000],center=true);
						}
