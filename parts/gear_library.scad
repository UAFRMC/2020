...
// Draw a rack with this many teeth (takes a geartype).
//   Teeth are arranged up the Y axis (for historical reasons).
//   Under teeth extends to "base" in the -X direction.
//   X==0 is the pressure plane. Y==0 is the peak of the first tooth.
module gear_rack_2D(gt,base,nteeth) {
	if (showteeth) {
		dT=0.0; //<- rack gear == infinite tooth gear
		Cpitch=geartype_Cpitch(gt);
		angle=geartype_pressure(gt);
		refR=-geartype_sub(gt);
		hO=geartype_add(gt)-refR;
		hM=0-refR;
		tilt = angle-0.5*dT;
		tI=Cpitch/4+geartype_sub(gt)*sin(tilt);
		tM=Cpitch/4;
		...
		// Add back in the base, for square corners:
		startY=-Cpitch/2;
		translate([-base,startY,0]) square([base+refR,Cpitch*nteeth]);
